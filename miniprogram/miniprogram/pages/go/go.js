...
var manager = plugin1.getRecordRecognitionManager(),
    ...
    t_str = '',
    [eat_list, play_list, buy_list, list, any_list] = [
        [],
        [],
        [],
        [],
    ...
    */
    ...
    search_place: async function (keyword, latitude, longitude) { //查找地点
        ...
        qqMap.search({
            ...
            auto_extend: 0,
            success: function (d) {
                if (keyword !== '购物' && keyword !== '娱乐' && keyword !== '餐饮') keyword = '随便'
                var res = d.data
                for (var i in res)
    ...
    go: function () {
        if (Math.max(places['娱乐'].length, places['购物'].length, places['餐饮'].length, places['随便'].length) < 13) {
            wx.showToast({
                title: '努力加载中',
                icon: 'loading'
            })
        ...
        this.check().then(() => {
            var list = play_list.concat(eat_list, buy_list, any_list)
            wx.setStorage({
                    data: places,
                    key: 'places',
                }),
    ...
    speak: function () {
        ...
        manager.onStop = function (res) {
            ...
            var keyword = res.result.slice(0, -1);
            that.search_place(keyword, that.data.latitude, that.data.longitude).then(() => setTimeout(function () {
                that.get_places('随便')
            }, 3000))
        }
